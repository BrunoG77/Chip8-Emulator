...
void init_chip8(Chip8& chip8, std::string_view rom_name){
    ...
    std::copy(FONT_SET.begin(), FONT_SET.end(), chip8.ram.begin() + FONTSET_START_ADDRESS);
    ...
    if (!rom.read(reinterpret_cast<char*>(chip8.ram.data() + ENTRY_POINT), rom_size)) {
    ...
    chip8.PC = ENTRY_POINT; // Program starts at 0x200
}
